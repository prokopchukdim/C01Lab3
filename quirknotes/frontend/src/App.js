...
function App() {
  ...
  const deleteAllNotes = () => {
    ...
    try {
      ...
      .then(async (response) => {
        if (!response.ok) {
          console.log("Delete all notes failed:", response.status);
  ...
  const patchNoteState = (_id, title, content) => {
    ...
    setNotes((prevNotes) => {
      let indx = prevNotes.findIndex(note => note._id === _id);
      prevNotes[indx].title = title;
      prevNotes[indx].content = content;
      return prevNotes;
    });
  }
